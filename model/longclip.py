...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu"):
    ...
    if not isinstance(state_dict, dict):
        raise RuntimeError(
            f"Expected checkpoint at {model_path} to contain a state_dict dictionary."
        )
    model = build_model(state_dict, load_from_clip=False).to(device)
...
def load_from_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None):
    ...
    # state_dict = torch.load(model_path, map_location="cpu", weights_only=True)ßß
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77*4-60, truncate: bool = True) -> Union[torch.IntTensor, torch.LongTensor]:
    """
...
def tokenizer(texts: Union[str, List[str]], context_length: int = 248, truncate_length: int = 248, truncate: bool = True) -> torch.Tensor:
    """
    Tokenizes input text(s), truncates to `truncate_length`, and ensures the output tensor has `context_length` dimensions.
    ...
    context_length : int
        The fixed output size of the tokenized sequence (default 248).

    truncate_length : int
        The length to truncate the tokenized sequence before padding (default 20).

    truncate: bool
        Whether to truncate the text if its tokenized length exceeds `truncate_length`.

    Returns
    -------
    A two-dimensional tensor of tokenized results, shape = [number of input strings, context_length].
    """
    if isinstance(texts, str):
        texts = [texts]
    ...
    for i, tokens in enumerate(all_tokens):
        # 截断到 truncate_length（确保最后一个是 eot_token）
        if len(tokens) > truncate_length:
            if truncate:
                tokens = tokens[:truncate_length]
                tokens[-1] = eot_token  # 确保最后一个 token 是 <|endoftext|>
            else:
                raise RuntimeError(f"Input {texts[i]} is too long for truncation length {truncate_length}")

        # 复制 token 到 result，并保持 248 维
        result[i, :len(tokens)] = torch.tensor(tokens)
